...
class VendingMachine:
    ...
    def sell(self, card_id, drink_name):
        card = None
        ...
        selected_line = None
        for line in self.line_list:
            if line.beverage and line.beverage.name == drink_name and line._number_beverage > 0:
                selected_line = line
                break

        if selected_line == None:
            return -1.0 
        beverage_price = selected_line.get_price(drink_name)
        if card._credit < beverage_price:
            return -1.0  

        card._credit -= beverage_price
        selected_line._number_beverage -= 1
        return selected_line._number 